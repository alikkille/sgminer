...
$mobilepage = [
 'DATE'        => null,
 'RIGS'        => null,
 'SUMMARY'     => ['Elapsed', 'MHS av', 'Found Blocks=Blks', 'Accepted', 'Rejected=Rej', 'Utility'],
 'DEVS+NOTIFY' => ['DEVS.Name=Name', 'DEVS.ID=ID', 'DEVS.Status=Status', 'DEVS.Temperature=Temp',
            'DEVS.MHS av=MHS av', 'DEVS.Accepted=Accept', 'DEVS.Rejected=Rej',
            'DEVS.Utility=Utility', 'NOTIFY.Last Not Well=Not Well', ],
 'POOL' => ['POOL', 'Status', 'Accepted', 'Rejected=Rej', 'Last Share Time'], ];
$mobilesum = [
 'SUMMARY'     => ['MHS av', 'Found Blocks', 'Accepted', 'Rejected', 'Utility'],
 'DEVS+NOTIFY' => ['DEVS.MHS av', 'DEVS.Accepted', 'DEVS.Rejected', 'DEVS.Utility'],
 'POOL'        => ['Accepted', 'Rejected'], ];
//
$statspage = [
 'DATE'    => null,
 'RIGS'    => null,
 'SUMMARY' => ['Elapsed', 'MHS av', 'Found Blocks=Blks',
...
//
$statssum = [
 'SUMMARY' => ['MHS av', 'Found Blocks', 'Accepted',
...
$poolspage = [
 'DATE'    => null,
 'RIGS'    => null,
 'SUMMARY' => ['Elapsed', 'MHS av', 'Found Blocks=Blks', 'Accepted', 'Rejected=Rej',
            'Utility', 'Hardware Errors=HW Errs', 'Network Blocks=Net Blks',
            'Work Utility', ],
 'POOL+STATS' => ['STATS.ID=ID', 'POOL.URL=URL', 'POOL.Difficulty Accepted=Diff Acc',
...
$poolssum = [
 'SUMMARY' => ['MHS av', 'Found Blocks', 'Accepted',
            'Rejected', 'Utility', 'Hardware Errors',
            'Work Utility', ],
 'POOL+STATS' => ['POOL.Difficulty Accepted', 'POOL.Difficulty Rejected',
...
$poolsext = [
 'POOL+STATS' => [
    'where' => null,
    'group' => ['POOL.URL', 'POOL.Has Stratum', 'POOL.Stratum Active', 'POOL.Has GBT'],
    'calc'  => ['POOL.Difficulty Accepted' => 'sum', 'POOL.Difficulty Rejected' => 'sum',
            ...
            'POOL.Accepted'                => 'sum', ],
    'gen'    => ['AvShr' => 'round(POOL.Difficulty Accepted/max(POOL.Accepted,1)*100)/100'],
    'having' => [['STATS.Bytes Recv', '>', 0]], ],
];

//
// customsummarypages is an array of these Custom Summary Pages
$customsummarypages = ['Mobile' => [$mobilepage, $mobilesum],
 'Stats'                        => [$statspage, $statssum],
 'Pools'                        => [$poolspage, $poolssum, $poolsext], ];
//
...
function getsock($rig, $addr, $port)
{
    ...
    socket_set_option($socket, SOL_SOCKET, SO_SNDTIMEO, ['sec' => $socksndtimeoutsec, 'usec' => 0]);
    socket_set_option($socket, SOL_SOCKET, SO_RCVTIMEO, ['sec' => $sockrcvtimeoutsec, 'usec' => 0]);
...
function revert($str)
{
    return str_replace(["\1", "\2", "\3", "\4"], ['|', '\\', '=', ','], $str);
}
//
...
function fmt($section, $name, $value, $when, $alldata)
{
    ...
    case 'total.Utility':
        ...
        if ($value == 0) {
            $class = $errorclass;
        } elseif (isset($alldata['Difficulty Accepted'])
    ...
    case 'DEVS.Fan Speed':
        if ($value == 0) {
            $class = $warnclass;
        } elseif (isset($alldata['Fan Percent'])) {
...
function details($cmd, $list, $rig)
{
    ...
    $stas = ['S' => 'Success', 'W' => 'Warning', 'I' => 'Informational', 'E' => 'Error', 'F' => 'Fatal'];
...
function gpubuttons($count, $rig)
{
    ...
    $basic = ['GPU', 'Enable', 'Disable', 'Restart'];
...
function showpoolinputs($rig, $ans)
{
    ...
    $inps = ['Pool URL'      => ['purl', 20],
        'Worker Name'     => ['pwork', 10],
        'Worker Password' => ['ppass', 10], ];
    $b = '&nbsp;';
...
{
    $dthead = [$headname => 1, 'STATUS' => 1, 'Description' => 1, 'When' => 1, 'API' => 1, 'sgminer' => 1];
    showhead('', $dthead);
...
function doOne($rig, $preprocess)
{
    ...
    if ($preprocess != null) {
        process([$preprocess => $preprocess], $rig);
    }

    $cmds = ['devs'    => 'device list',
...
function display()
{
    ...
    newtable();
    doforeach('version', 'rig summary', [], [], true);
    $sum = ['MHS av', 'Getworks', 'Found Blocks', 'Accepted', 'Rejected', 'Discarded', 'Stale', 'Utility', 'Local Work', 'Total MH', 'Work Utility', 'Diff1 Shares', 'Diff1 Work', 'Difficulty Accepted', 'Difficulty Rejected', 'Difficulty Stale'];
    doforeach('summary', 'summary information', $sum, [], false);
    endtable();
    otherrow('<td><br><br></td>');
    newtable();
    doforeach('devs', 'device list', $sum, [''=>'', 'ID'=>'', 'Name'=>''], false);
    endtable();
    otherrow('<td><br><br></td>');
    newtable();
    doforeach('pools', 'pool list', $sum, [''=>''], false);
    endtable();
